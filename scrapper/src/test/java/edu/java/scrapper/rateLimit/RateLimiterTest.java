...
public class RateLimiterTest extends IntegrationTest{
    ...
    public void getAll_whenEndpointIsRestrictedAfter3RequestsInAShortTime() throws Exception {
        ...
        for (int i = 0; i < 10; i++) {
            mockMvc.perform(get("/links").header("tgChatId", chatId)).andExpect(status().isOk());
        }

        mockMvc.perform(get("/links").header("tgChatId", chatId)).andExpect(status().isTooManyRequests());
    }
}
